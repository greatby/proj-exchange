export default function StatsSection() {
  ...
  return (
    ...
    <section className="w-full bg-[#F9FAF5] pt-20 px-6 md:px-12 text-center flex flex-col items-center">
      {/* Heading */}
      <h2 className="text-3xl md:text-5xl font-[Monotype] font-medium text-[#003E47] max-w-4xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-16 w-full max-w-6xl">
        {stats.map((item, index) => (
          <div
            key={index}
            className={`${item.bg} rounded-xl py-10 px-6 flex flex-col items-center text-center shadow-md border border-[#E5E7EB]/40`}
          >
            <h3 className="text-[2.75rem] font-medium text-[#003E47]">{item.value}</h3>
            <p className="text-gray-800 mt-3 text-[1.125rem] font-normal leading-snug">{item.label}</p>
          </div>
        ))}
      </div>
      ...
      <p className="text-[11px] px-24 text-left text-gray-600 mt-2 max-w-4xl leading-relaxed">
